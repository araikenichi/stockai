...
ipcMain.handle('run-lite', async(_,{keys,symbol,marketData,tvState,screenshot,lang,userInstruction,portfolio})=>{
  ...
  try{
    ...
    rawTexts.E=await callAI(type,activeKey,mdl,[{role:'system',content:`CEO秘書・研究統合AI。${li}クオンツ・リスク分析を統合して研究レポートを作成。個人向け売買指示・断定的な価格予測・必ず儲かる表現は禁止。\n\n【必須項目】\n1. 結論は「上がる/下がる」ではなく、必ず「確率 + 条件」で書く。\n2. probability は控えめに 35-85 の範囲で出す。100%は禁止。\n3. keyCondition は判断が確認される条件、invalidationCondition は判断を取り消す条件。\n4. 予測は断定せず「注目ポイント」形式で短中長期。\n\nJSON:{"finalVerdict":"BULLISH/BEARISH/NEUTRAL/WATCH","probability":N(35-85),"confidence":N(0-100),"score":N(1-10),"summary":"...2-3文の核心まとめ","keyReasons":["具体的根拠3-5件..."],"actionPlan":"次に確認すべき情報・観察ポイント","entryCondition":"具体的な価格・指標条件（例:$XXX以上かつRSI50超）","keyCondition":"突破/反発/出来高など、判断が確認される条件","invalidationCondition":"跌破/決算悪化/出来高失速など、判断を取り消す条件","avoidEvents":["..."],"riskWarning":"...","timeHorizon":"SHORT|MEDIUM|LONG|NEUTRAL","riskLevel":"LOW|MEDIUM|HIGH|CRITICAL","whyMattersToUser":"持ち株・ウォッチとの関連性（1-2文）","prediction":{"shortTerm":"1-2週間の注目点（具体的水準を含む）","midTerm":"1-3ヶ月の注目点（触媒を含む）","longTerm":"6-12ヶ月の注目点（テーマを含む）"}}`},{role:'user',content:eC}],false);
